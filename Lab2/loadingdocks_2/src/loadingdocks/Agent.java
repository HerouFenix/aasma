...
public class Agent extends Entity {
    ...
    public void agentDecision() {
        ...
        if (cargo()) {
            if (isShelf() && !isBoxAhead() && cargoColor().equals(shelfColor())) {
                dropBox();
                return;
            }
        } else {
            if (isRamp() && isBoxAhead()) {
                grabBox();
                return;
            }
        }
        ...
        if (!isFreeCell()) {
        	...
        	if(random.nextInt(10) == 0){ // 1 in 10 chance that we just rotate randomly (done so that we don't get stuck)
        		rotateRandomly();
        		return;
			}
    ...
    public Color shelfColor() {
        if (Board.getBlock(ahead).shape.equals(Shape.shelf)) {
    ...
    public void rotateRandomly() {
        if (this.random.nextBoolean()) rotateLeft();
        else rotateRight();
    }
